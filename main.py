...
def dedust_extract_buys_from_tonapi_tx(tx: Dict[str, Any], pool: str) -> List[Dict[str, Any]]:
    """
    Extract BUY swaps from TonAPI tx actions for DeDust pools.
    ...
    for a in actions:
        ...
        # Filter for DeDust if dex field exists
        dex = a.get("dex")
        dex_name = ""
        if isinstance(dex, dict):
            dex_name = str(dex.get("name") or dex.get("title") or dex.get("id") or "").lower()
        if dex_name and "dedust" not in dex_name:
            continue
        ...
        if ton_in is not None:
            s = str(ton_in)
            ton_spent = to_ton_from_nano(ton_in) if s.isdigit() else safe_float(ton_in)

        # Jetton out + master for decimals
        jet_out = a.get("jetton_out") or a.get("jettonOut") or a.get("out_jetton") or a.get("outJetton") or a.get("amount_jetton_out")
        out_master = (
            a.get("jetton_master")
            or a.get("jettonMaster")
            or a.get("jetton_out_master")
            or a.get("jettonOutMaster")
            or a.get("jetton")
        )
        if isinstance(out_master, dict):
            out_master = out_master.get("address") or out_master.get("master") or out_master.get("jetton_master") or out_master.get("id")

        token_received = 0.0
        if jet_out is not None:
            s = str(jet_out)
            if s.isdigit():
                dec = get_jetton_decimals(str(out_master)) if out_master else 9
                token_received = float(int(s)) / (10 ** int(dec))
            else:
                token_received = safe_float(jet_out)
    ...
    async def _send_message(chat_id: int):
        ...
        if chat_id == MASTER_CHANNEL_ID:
            ...
            return
                ...
                # GROUPS: one token per group + per-group customization
        cfg = _ensure_group_cfg(chat_id)

        # Skip if group hasn't configured any token
        ...
        try:
            ...
            min_buy_usd = 0.0
        # back-compat: if only TON threshold exists
        try:
            if (not min_buy_usd) and float(cfg.get("min_buy_ton") or 0.0) > 0 and ton_usd > 0:
                min_buy_usd = float(cfg.get("min_buy_ton") or 0.0) * float(ton_usd)
        except:
            pass
        if min_buy_usd and usd_amt < min_buy_usd:
            return

        # Apply emoji + custom links line
        grp_emoji = str(cfg.get("emoji") or "💡")
        tg_u = cfg.get("telegram") or tg_url
        links_line = _group_links_line(tg_u, chart_url)
        extra_link = cfg.get("custom_link")
        if extra_link:
            links_line = links_line + f" | <a href='{extra_link}'>Link</a>"

        # Build group message (similar to your screenshot)
        dex_lbl_plain = (lbl or source_label or "DEX").strip() or "DEX"
        grp_pos = "New!" if "new" in (pos_txt or "").lower() else "Old!"
        price_val = stats.get("price_usd")
        price_txt = f"{price_val:.6f}".rstrip("0").rstrip(".") if isinstance(price_val, (int, float)) and price_val > 0 else "—"
        ...
        # Strength bar: each emoji represents `buy_step` TON
        try:
            step = float(cfg.get("buy_step") or 1.0)
            if step <= 0:
                step = 1.0
        except:
            step = 1.0
        n = int(ton_amt / step) if ton_amt > 0 else 1
        n = max(1, min(60, n))
        bar = (str(cfg.get("emoji") or "🟢") * n)
        per_line = 12
        bar_lines = "\n".join([bar[i:i+per_line] for i in range(0, len(bar), per_line)])

        # Suite-style group message (like your SUITE screenshot)
        wallet_html = f"<a href='{tx_url}'>{buyer_group}</a>" if tx_url else buyer_group
        txn_html = f"<a href='{tx_url}'>Txn</a>" if tx_url else "Txn"
        group_msg = (
            f"<b>{sym_g} Buy!</b>\n\n"
            f"{bar_lines}\n\n"
            f"💧 <b>{ton_amt:.2f} TON</b> ({'$' + usd_group if usd_group else '$0'})\n"
            f"💰 <b>{token_amt:,.2f} {sym_g}</b>\n\n"
            f"{wallet_html}: <b>{grp_pos}</b> | {txn_html}\n"
            f"Price: <b>${price_txt}</b>\n"
            f"Liquidity: <b>${liq_group}</b>\n"
            f"MCap: <b>${mc_group}</b>\n"
            f"TON Price: <b>${ton_usd:.2f}</b>\n\n"
            f"{links_line}"
        )

        dtrade_url = _group_dtrade_url(cfg.get("token_address") or token_addr)
        kb = InlineKeyboardMarkup([[InlineKeyboardButton(f"Buy {sym_g} on DTrade", url=dtrade_url)]])

        # Optional media/logo per group
        if cfg.get("media_file_id"):
            try:
                msg = await context.bot.send_photo(
                    chat_id=chat_id,
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Private chat:
      - shows menu buttons
      - supports deep-link: /start cfg_<group_id>
    Group chat:
      - shows "connected" + button to configure in private
    """
    chat = update.effective_chat
    uid = update.effective_user.id if update.effective_user else 0

    # Parse deep-link payload (private only)
    payload = ""
    try:
        if context.args:
    ...
    if chat and chat.type == "private":
        ...
        if payload.startswith("cfg_"):
            try:
                gid = int(payload[4:])
                context.user_data["cfg_group"] = gid
            except:
                pass

        gid = context.user_data.get("cfg_group")
        bot_username = await get_bot_username(context)

        # Wizard auto-start (video-like setup)
        if gid:
            cfg0 = _ensure_group_cfg(int(gid))
            if not cfg0.get("token_address"):
                context.user_data["wizard"] = {"group_id": int(gid), "step": "ca"}
                await update.message.reply_text(
                    "🛠 Configure Token\n\nPaste your token contract address (Jetton master).",
                    disable_web_page_preview=True
                )
                return
        ...
        title = "🟢 SpyTON BuyBot"
        if gid:
            title += f"\n\n✅ Group selected: <code>{gid}</code>"
            # Permission hint
            title += "\nOnly that group's admins can change settings."

        kb = []

        # Add to group button
        if bot_username:
            kb.append([InlineKeyboardButton("➕ Add BuyBot to Group", url=f"https://t.me/{bot_username}?startgroup=true")])

        # Actions
        kb.append([
            InlineKeyboardButton("➕ Add Token", callback_data="menu:addtoken"),
            InlineKeyboardButton("⚙️ Edit Token", callback_data="menu:edit"),
        ])
        kb.append([
            InlineKeyboardButton("📌 My Token", callback_data="menu:tokens"),
            InlineKeyboardButton("🗑 Remove Token", callback_data="menu:remove"),
        ])
        ...
        help_txt = (
            ...
            "2) In your group, run /start (or open here and select the group)\n"
            "3) Add token with /addtoken <JETTON_ADDRESS> <SYMBOL> [TELEGRAM_LINK]\n\n"
            "Commands (admins):\n"
            "/addtoken <JETTON_ADDRESS> <SYMBOL> [TELEGRAM_LINK]\n"
            "/removetoken\n"
            "/tokens\n"
            "/edittoken\n"
        )
        await update.message.reply_text(help_txt, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(kb), disable_web_page_preview=True)
        return
    ...
    if chat and chat.type in ("group", "supergroup"):
        ...
        cfg_url = None
        if bot_username:
            cfg_url = f"https://t.me/{bot_username}?start=cfg_{chat.id}"

        txt = (
            "✅ Your group has been connected to SpyTON BuyBot.\n"
            "You can now configure your token to start forwarding buys here.\n\n"
            "1) Click “Configure Token”\n"
            "2) Add token with /addtoken <JETTON_ADDRESS> <SYMBOL> [TELEGRAM_LINK]\n"
            "3) Customize with /edittoken\n"
        )
        kb = []
        if cfg_url:
            kb = [[InlineKeyboardButton("⚙️ Configure Token", url=cfg_url)]]
        await update.message.reply_text(txt, reply_markup=InlineKeyboardMarkup(kb) if kb else None, disable_web_page_preview=True)
        return

    # Fallback
    await update.message.reply_text("🟢 SpyTON BuyBot is running.")


# =========================
# Multi-Group Support (one token per group)
# =========================
...
def _effective_group_id(update: Update, context: ContextTypes.DEFAULT_TYPE) -> Optional[int]:
    """
    If called inside a group => that group.
    If called in private with deep-link cfg_<group_id> => that group.
    """
    chat = update.effective_chat
    if chat and chat.type in ("group", "supergroup"):
        return chat.id
...
async def _require_group_admin(update: Update, context: ContextTypes.DEFAULT_TYPE, group_id: int) -> bool:
    """
    Allow:
      - global ADMIN_ID
...
def _ensure_group_cfg(cid: int) -> Dict[str, Any]:
    """
    Creates default config structure for the group if missing.
    Stored in DATA['group_mirrors'][cid] (single token + settings).
    """
    load_data()
    DATA.setdefault("group_mirrors", {})
    cfg = DATA["group_mirrors"].get(str(cid))
    if not isinstance(cfg, dict):
        cfg = {}
        DATA["group_mirrors"][str(cid)] = cfg
    ...
    cfg.setdefault("updated_ts", int(time.time()))

    # Customization defaults
    cfg.setdefault("buy_step", 1.0)          # controls strength bar in groups
    cfg.setdefault("min_buy_usd", 0.0)       # skip buys smaller than this (USD)
    cfg.setdefault("min_buy_ton", 0.0)       # legacy (TON), kept for back-compat
    cfg.setdefault("emoji", "💡")            # group highlight emoji (used in message)
    cfg.setdefault("custom_link", None)      # optional extra link shown in message
    cfg.setdefault("media_file_id", None)    # optional photo file_id
    cfg.setdefault("media_type", "photo")

    save_data()
    return cfg

def _group_dtrade_url(token_addr: Optional[str]) -> str:
    # user requested: https://t.me/dtrade?start=11TYq7LInG + token ca
    base = "https://t.me/dtrade?start=11TYq7LInG"
    if token_addr:
        # Telegram deep-link uses underscore separator commonly
        return f"{base}_{token_addr}"
    return base

def _group_links_line(tg_url: Optional[str], chart_url: str) -> str:
    # Always include Trending
    tg_part = f"<a href='{tg_url}'>Telegram</a>" if tg_url else "Telegram"
    dexs_part = f"<a href='{chart_url}'>DexS</a>"
...
async def tokens_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not cid:
        await update.message.reply_text("Use this inside a group, or open /start in private via Configure Token.")
        return

    # Privacy: in private chat, only that group's admins can view its token.
    chat = update.effective_chat
    if chat and chat.type == "private":
        if not await _require_group_admin(update, context, int(cid)) and not is_admin(update.effective_user.id if update.effective_user else 0):
            return

    cfg = _ensure_group_cfg(cid)
    sym = cfg.get("symbol")
    addr = cfg.get("token_address")
    if not sym or not addr:
        await update.message.reply_text("No token set for this group yet. Use /addtoken <JETTON_ADDRESS> <SYMBOL> [TELEGRAM_LINK]")
        return
    tg = cfg.get("telegram")
    dex = cfg.get("dex") or "pending"
    pair = cfg.get("pair_id") or "pending"
    await update.message.reply_text(
        f"✅ This group token\n"
        f"Symbol: <b>{html.escape(str(sym))}</b>\n"
        f"Address: <code>{addr}</code>\n"
        f"DEX: <b>{html.escape(str(dex))}</b>\n"
        f"Pair/Pool: <code>{pair}</code>\n"
        f"Telegram: {tg or 'NONE'}",
        parse_mode="HTML",
        disable_web_page_preview=True,
    )
...
async def removetoken(update: Update, context: ContextTypes.DEFAULT_TYPE):
    cid = _effective_group_id(update, context)
    if not cid:
        await update.message.reply_text("Use this inside your group (or via Configure Token in private).")
        return
    ...
    load_data()
    mirrors = DATA.get("group_mirrors", {})
    if isinstance(mirrors, dict) and str(cid) in mirrors:
        mirrors[str(cid)] = _ensure_group_cfg(cid)
        # Clear token fields only (keep customization)
        mirrors[str(cid)]["symbol"] = None
        mirrors[str(cid)]["token_address"] = None
        mirrors[str(cid)]["pair_id"] = None
        mirrors[str(cid)]["dex"] = None
        mirrors[str(cid)]["telegram"] = None
        mirrors[str(cid)]["updated_ts"] = int(time.time())
        save_data()
        await update.message.reply_text("🗑 Removed token for this group.")
        return
    await update.message.reply_text("No token set for this group.")

async def edittoken(update: Update, context: ContextTypes.DEFAULT_TYPE):
    cid = _effective_group_id(update, context)
    if not cid:
        await update.message.reply_text("Use this inside your group (or via Configure Token in private).")
        return
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("menu:"):
        ...
        if action == "addtoken":
            ...
            dummy_update = update
            cid = _effective_group_id(dummy_update, context)
            if not cid:
                await q.message.reply_text("First select a group: open Configure Token from your group and click it.")
                return
            if not await _require_group_admin(update, context, cid):
                return
            cfg = _ensure_group_cfg(cid)
            ...
            # Show token
            fake = update
            cid = _effective_group_id(fake, context)
            if not cid:
                await q.message.reply_text("Select a group first.")
                return
            cfg = _ensure_group_cfg(cid)
            sym = cfg.get("symbol"); addr = cfg.get("token_address")
            if not sym or not addr:
                await q.message.reply_text("No token set yet. Use /addtoken ...")
            else:
                await q.message.reply_text(f"Token: {sym}\nAddress: {addr}")
        elif action == "remove":
            cid = _effective_group_id(update, context)
            if not cid:
                await q.message.reply_text("Select a group first.")
            ...
            if not await _require_group_admin(update, context, cid):
                return
            cfg = _ensure_group_cfg(cid)
            cfg["symbol"]=None; cfg["token_address"]=None; cfg["pair_id"]=None; cfg["dex"]=None; cfg["telegram"]=None; cfg["updated_ts"]=int(time.time())
            save_data()
            await q.message.reply_text("🗑 Removed token for this group.")
        return
    ...
    if data.startswith("edit:set:"):
        ...
        if not await _require_group_admin(update, context, cid):
            return
        field = data.split(":")[-1]
        # normalize legacy button
        if field == "min_buy_usd":
            field = "min_buy_usd"
        context.user_data["pending_edit"] = {"group_id": cid, "field": field}
        prompts = {
            "buy_step": "Send a number for Buy Step (example: 1, 2, 5).",
            "min_buy_usd": "Send Min Buy in USD (example: 25). Buys smaller than this won't be posted.",
            "min_buy_ton": "Send Min Buy in TON (legacy).",
            "emoji": "Send the emoji you want (example: 💡 or 🟢).",
            "custom_link": "Send the extra link (or 'none' to clear).",
            "media": "Send the photo/logo you want the buy to display (as a photo message).",
        }
        await q.message.reply_text(prompts.get(field, "Send the new value."))
        return

    if data == "edit:clear:media":
        cid = _effective_group_id(update, context)
        if not cid:
            return
        if not await _require_group_admin(update, context, cid):
        ...
        cfg["updated_ts"] = int(time.time())
        save_data()
        await q.message.reply_text("✅ Media removed.")
        return
    ...
    if data == "edit:preview":
        ...
        if not await _require_group_admin(update, context, cid):
            return
        cfg = _ensure_group_cfg(cid)
        sym = cfg.get("symbol") or "TOKEN"
        addr = cfg.get("token_address") or ""
        dtrade_url = _group_dtrade_url(addr)
        chart_url = f"https://www.geckoterminal.com/ton/tokens/{addr}" if addr else "https://geckoterminal.com"
        tg_url = cfg.get("telegram")
        links_line = _group_links_line(tg_url, chart_url)
        preview = (
            f"🚀 {sym} TOKEN Buy! — PREVIEW\n"
            f"✅ LISTED!\n\n"
            f"{cfg.get('emoji','💡')*10}\n\n"
            f"💰 1.00 TON ($0)\n"
            f"📦 1,000 {sym}\n"
        ...
        kb = InlineKeyboardMarkup([[InlineKeyboardButton(f"Buy {sym} on DTrade", url=dtrade_url)]])
        if cfg.get("media_file_id"):
            try:
                await q.message.reply_photo(photo=cfg["media_file_id"], caption=preview, parse_mode="HTML", reply_markup=kb)
                return
            except Exception:
                pass
        await q.message.reply_text(preview, parse_mode="HTML", reply_markup=kb, disable_web_page_preview=True)
        return

    if data == "edit:test":
        await q.message.reply_text("Use preview. Real buys will be posted automatically.")
        return

    if data == "edit:return":
        await q.message.reply_text("✅ Returned.")
        return
...
async def on_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Captures replies during edit flow:
      - text values for buy_step/min_buy/emoji/link
      - photo for media
    """
    pending = context.user_data.get("pending_edit")
    if not pending or not isinstance(pending, dict):
        # Wizard (private setup flow)
        wiz = context.user_data.get("wizard")
        chat = update.effective_chat
        if wiz and isinstance(wiz, dict) and chat and chat.type == "private":
            # Ignore commands
            if update.message and (update.message.text or "").strip().startswith("/"):
                return
            gid = int(wiz.get("group_id") or 0)
            step = (wiz.get("step") or "").strip()

            # Permission: only group admins can configure
            if gid and not await _require_group_admin(update, context, gid) and not is_admin(update.effective_user.id if update.effective_user else 0):
                return
            ...
            if step == "ca":
                ...
                if not ca or len(ca) < 32:
                    await update.message.reply_text("❌ Please paste a valid contract address.")
                    return

                sym = None
                try:
                    js = tonapi_get(f"{TONAPI_BASE.rstrip('/')}/v2/jettons/{ca}")
                    if isinstance(js, dict):
                        md = js.get("metadata") if isinstance(js.get("metadata"), dict) else {}
                        sym = (md.get("symbol") or md.get("ticker") or md.get("name"))
                except Exception:
                    sym = None

                sym = (str(sym).strip() if sym else None)
                if sym:
                    sym = sym.replace("$", "").strip()
                    if len(sym) > 16:
                        sym = sym[:16]

                pair_id = find_stonfi_ton_pair_for_token(ca)
                dex = "stonfi" if pair_id else None
                if not pair_id:
                    pair_id = find_dedust_ton_pair_for_token(ca)
                ...
                wiz["step"] = "min_buy_usd"
                context.user_data["wizard"] = wiz

                await update.message.reply_text(
                    f"✅ Token saved for this group.\nSymbol: <b>{cfg['symbol']}</b>\n\nNow send Min Buy in USD (example: 25)",
                    parse_mode="HTML"
                )
                return
            ...
            # STEP 2: Min buy USD
            if step == "min_buy_usd":
                val = (update.message.text or "").strip() if update.message else ""
                try:
                    f = float(val.replace(",", ""))
                    if f < 0:
                        raise ValueError()
                    ...
                    await update.message.reply_text("❌ Send a valid USD number like 25")
                    return
                wiz["step"] = "buy_step"
                context.user_data["wizard"] = wiz
                await update.message.reply_text("✅ Min Buy saved.\nNow send Buy Step (example: 1, 2, 5)")
                return
            ...
            if step == "buy_step":
                ...
                try:
                    ...
                    await update.message.reply_text("❌ Send a valid number like 1 or 2")
                    return
                wiz["step"] = "emoji"
                context.user_data["wizard"] = wiz
                await update.message.reply_text("✅ Buy Step saved.\nNow send your Emoji (example: 💡 or 🟢)")
                return
            ...
            if step == "emoji":
                val = (update.message.text or "").strip() if update.message else ""
                if not val:
                    await update.message.reply_text("❌ Send an emoji.")
                    return
                cfg["emoji"] = val[:2]
                cfg["updated_ts"] = int(time.time())
                save_data()
                wiz["step"] = "media"
                context.user_data["wizard"] = wiz
                await update.message.reply_text("✅ Emoji saved.\nSend your logo/photo now, or type SKIP.")
                return

            # STEP 5: Media (photo optional)
            if step == "media":
                photo = None
                try:
                ...
                if photo:
                    cfg["media_file_id"] = photo.file_id
                    cfg["media_type"] = "photo"
                    cfg["updated_ts"] = int(time.time())
                    save_data()
                else:
                    val = (update.message.text or "").strip().lower() if update.message else ""
                    if val not in ("skip", "no", "none", "0", ""):
                        await update.message.reply_text("Send a photo/logo, or type SKIP.")
                        return
    ...
    # Permission check
    if not await _require_group_admin(update, context, int(cid)):
        context.user_data.pop("pending_edit", None)
        return

    cfg = _ensure_group_cfg(int(cid))
    ...
    if field == "media":
        ...
        try:
            ...
            photo = None
        if not photo:
            await update.message.reply_text("Please send a photo/logo.")
            return
        cfg["media_file_id"] = photo.file_id
        cfg["media_type"] = "photo"
        cfg["updated_ts"] = int(time.time())
    ...
    if field == "custom_link":
        if val.lower() in ("none", "no", "clear", "0"):
            cfg["custom_link"] = None
        else:
            cfg["custom_link"] = normalize_url(val)
        cfg["updated_ts"] = int(time.time())
        save_data()
        context.user_data.pop("pending_edit", None)
    ...
    if field == "min_buy_usd":
        try:
            ...
            await update.message.reply_text("Send a valid number like 0.5")
        return
    # Legacy TON threshold (kept for older configs/buttons)
    if field == "min_buy_ton":
        try:
            f = float(val)
            if f < 0:
                raise ValueError()
            ...
            save_data()
            context.user_data.pop("pending_edit", None)
            await update.message.reply_text("✅ Min Buy (TON) saved.")
        except:
            await update.message.reply_text("Send a valid number like 0.5")
        return
...
async def addtoken(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Permission:
    # - global ADMIN_ID can always add tokens
    # - group admins can configure ONLY their group (one token per group)
    chat = update.effective_chat
    uid = update.effective_user.id if update.effective_user else 0

    # Resolve target group if in private (deep-link cfg_<gid>)
    target_gid = _effective_group_id(update, context)

    if not is_admin(uid):
        if chat and chat.type in ("group", "supergroup"):
            if not await is_chat_admin(context, chat.id, uid):
                return
        elif chat and chat.type == "private" and target_gid:
            if not await _require_group_admin(update, context, target_gid):
                return
        else:
            return
    ...
    if not token_address:
        # In groups (or private configuring a group) we require jetton master address
    ...
    if not pair_id:
        ...
        if (chat and chat.type in ("group", "supergroup")) or (chat and chat.type=="private" and target_gid):
            gid = chat.id if (chat and chat.type in ("group","supergroup")) else int(target_gid)
            # One token per group: overwrite token fields, keep customization.
            cfg = _ensure_group_cfg(gid)
            cfg["symbol"] = symbol
...
async def dedust_tracker_job(context: ContextTypes.DEFAULT_TYPE):
    """
    Poll DeDust pool transactions via TonAPI and post BUY-only swaps.
    Fixes:
      - No backfill on first run per pool (prevents old tx repost)
      - Persistent dedupe across restarts (STATE['dedust_seen'])
      - Cursor uses LT per pool (STATE['dedust_last_lt'])
    """
    global LAST_HTTP_INFO, LAST_EVENTS_COUNT
    ...
    async with DEDUST_POLL_LOCK:
        try:
            ...
            # state maps
            if not isinstance(STATE.get("dedust_last_lt"), dict):
                STATE["dedust_last_lt"] = {}
            last_lt_map: Dict[str, int] = STATE.get("dedust_last_lt", {})

            if not isinstance(STATE.get("dedust_seen"), dict):
                STATE["dedust_seen"] = {}
            seen_persist: Dict[str, float] = STATE.get("dedust_seen", {})
            ...
            for pool, rec in pools.items():
                ...
                pair_id = pool
                if isinstance(rec, dict):
                    token_addr = rec.get("token") or rec.get("token_address") or rec.get("jetton_master")
                    sym = rec.get("symbol") or rec.get("sym")
                token_addr = str(token_addr or "").strip()
                sym = str(sym or "").strip() or "TOKEN"

                txs = tonapi_account_transactions(pool, limit=DEDUST_POLL_LIMIT)
                if not txs:
                    continue

                txs_sorted = sorted(txs, key=lambda t: _tx_lt(t))
                newest_lt = _tx_lt(txs_sorted[-1])
                last_lt = int(last_lt_map.get(pool) or 0)

                # First run: set cursor and DO NOT post old
                if last_lt == 0 and newest_lt > 0:
                    last_lt_map[pool] = newest_lt
                    continue
                ...
                for tx in txs_sorted:
                    ...
                    buys = dedust_extract_buys_from_tonapi_tx(tx, pool)
                    if not buys:
                        continue

                    for b in buys:
                        txh = str(b.get("tx") or "")
                        dedupe_key = f"{pool}:{txh}"
                        if txh and dedupe_key in seen_persist:
                            continue
                        if txh:
                            seen_persist[dedupe_key] = time.time()
            ...
            LAST_HTTP_INFO = f"DeDust TonAPI OK new={total_new}"
...
def main():
    ...
    while True:
        try:
            ...
            bot.add_handler(CommandHandler("tokens", tokens_cmd))
            bot.add_handler(CommandHandler("removetoken", removetoken))
            bot.add_handler(CommandHandler("remove_token", removetoken))
            bot.add_handler(CommandHandler("remove", removetoken))
